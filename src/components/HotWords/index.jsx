...
function HotWords (props) {
    ...
    return (
        <div id="hotwords">
            {/*<Typography.Title level={5}>大家标记为：</Typography.Title>*/}
            {/*<div style={{margin:'2rem auto 2rem auto'}}>*/}
            {/*    /!*<Tags tags={hotWords} history={props.history}/>*!/*/}
            {/*</div>*/}
            {
                // hotWords.map(item => <Link to={{pathname:`/detailInfo/${item.guid}`}}>{item.name}</Link>)
                hotWords.map(
                    item =>
                        <Tag style={{marginLeft:'5px', marginTop:'30px'}} color={generateRandomColor()} onClick={() => {
